...
async function fetchFollowers(userHandle: string): Promise<{ user: User | null; followers: User[] }> {
  ...
  const followerUsers = (followers?.map((f) => f.follower).filter(Boolean) || []) as User[]

  return { user, followers: followerUsers }
}
