...
export default function useSearch(): UseSearchResult {
  ...
  const search = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (usersResult.data) {
        setUsers(usersResult.data.map((u: Record<string, unknown>) => ({
          id: u.id as string,
      ...
      if (worldsResult.data) {
        setWorlds(worldsResult.data.map((w: Record<string, unknown>) => ({
          id: w.id as string,
      ...
      if (postsResult.data) {
        setPosts(postsResult.data.map((p: Record<string, unknown>) => ({
          id: p.id as string,
